...
4-Retrieve all patient tests based on test status:\n"
	...
	do
		...
		2) # if the user choose 2 it will print all the patient normal test
			...
			do
				...
				if echo "$range" | grep -q ',' ; then
					# equlibrim with $(echo "$rane"|grep -q ',' ) " -eq 0 then
		...
		3) # if the user choose 3 it will print all the patient test in specific period
			...
			(sort -n time.txt | uniq ) > sort.txt			# save after sort the date and take the uniq so there is no repeated date
...
4) # if the user choose 4 it will print the average
	...
	sort name.txt | uniq  > sort.txt  # save after sort the names and take the uniq so there is no repeated names
...
#update case
	...
	#^$ID to ensure that contanin an id in the same way
	if [ $(cat id.txt | wc -l) -eq 0 ]; 
	#cheak if id.txt empty or not if it is empty that mean the person with id have no test
	then
	...
	cat id.txt #print all test for the peron with input id to be easer to the user work with data
	echo -e "\n____________\n"
	...
	#cheak if the name of test is contain a char only
	while [ $? -ne 0 ]
	do
	#repate until get the true input 
	...
	if [ "$line" -gt 1 ]  #we cheak if we have amultiple test for the same peron
	then 
		cat -n test.txt
		echo -e "\n____________\n"
	...
	else #if we have just one special spefict test for the person
		cat test.txt #repate the step in apper case but without ask the user which line
		echo -e "\n____________\n"
...
#delete case
	...
	echo "$ID" | grep -q '^[0-9]\+$' #cheak if the input value is accepted or not same way in update
	while [ $? -ne 0 ]
	do
		echo -e "Write the patient ID:"
		read ID
	...
	if [ $(wc -c < id.txt) -eq 0 ]; #cheak if id.txt empty or not if it is empty that mean the person with id have no test
	then
		echo "No tests found for the person with ID $ID."
		echo -e "____________\n"
	
	elif [ $(wc -l < id.txt) -eq 1 ]; #cheak if id.txt has one test if there one line then there is no need to ask the user about the line
	then
		cat  id.txt
		echo -e "____________\n"
		...
		read check
		echo -e "____________\n"
		if [ "$check" = "Y" ]   # if yes then it will delete the line 
		then
			line=$(cat id.txt)
			#stor the line in var named line
			(grep -v  "$line" medicalRecord.txt) > newRecord.txt
			#using grep we search about the line in our file and delet it and rewrite the date in new file 
			mv newRecord.txt  medicalRecord.txt
			#change the contain of the orignal line
			cat medicalRecord.txt
			echo -e "\ndelete file ended"
			echo -e "____________\n"
			rm id.txt
		...
		read number
		echo -e "____________\n"
		echo "$number" | grep -q '^[0-9]\+$'  #cheak if the input value is accepted or not
		while [ $? -ne 0 ]
		do
			echo -e "choose a line:"
			read number
		...
		read check
		echo -e "____________\n"
		if [ "$check" = "Y" ] # if yes then it will delete the line that user choose
		then
